...
<head>
    ...
    <title>{{ $artikel->judul }} - MedEase</title>
...
<div class="container" style="margin-top: 50px; margin-bottom: 200px;">
        ...
        <h2 style="margin-bottom: 50px;">{{ $artikel->judul }}</h2>
        @if ($artikel->gambar)
    <img src="{{ asset('storage/' . $artikel->gambar) }}" alt="Gambar Artikel" class="mx-auto d-block" style="max-width: 900px; max-height: 850px; margin-bottom: 50px;">
@endif
<p class="text-justify">{{ $artikel->konten }}</p>
        <span><i class="far fa-calendar-check" style="color: #0E5895; margin-top: 50px;"></i> {{ $artikel->created_at->format('d F Y') }}</span></br></br></br>
